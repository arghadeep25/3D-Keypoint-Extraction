...
#include"../include/RowColReader.h"
#include"../include/GrayValues.h"
#include"../include/GrayFileReader.h"
#include<math.h>
#include<string.h>
...
cv::Mat det_tensor(cv::Mat gradX, cv::Mat gradY, cv::Mat grad_XY) {
	...
	for (int i = 0;i < gradX.rows;i++) {
		for (int j = 0;j < gradY.cols;j++) {
			determinant_mat.at<float>(i, j) = (gradX.at<float>(i, j)*
                                         gradY.at<float>(i, j)) -
                                         (grad_XY.at<float>(i, j)*
                                         grad_XY.at<float>(i, j));
		}
	}
...
cv::Mat isotropy_img(cv::Mat det, cv::Mat trace) {
	...
	for (int i = 0;i < det.rows;i++) {
		for (int j = 0;j < det.cols;j++) {
			isotropy_mat.at<float>(i, j) = (4 * det.at<float>(i, j)) /
                                  (trace.at<float>(i, j)*trace.at<float>(i, j));
		}
	}
	return isotropy_mat;
}
...
vector<cv::KeyPoint> key_point(cv::Mat weight_ST, cv::Mat isotropy_ST, float weight_mean) {
	...
	for (int i = 0;i < weight_ST.rows;i++) {
		for (int j = 0;j < weight_ST.cols;j++) {
			if (weight_ST.at<float>(i, j) > weight_mean*0.5 &&
      weight_ST.at<float>(i, j) < weight_mean*1.5 &&
      isotropy_ST.at<float>(i, j) > 0.5 && isotropy_ST.at<float>(i, j) < 0.75) {
...
//Keypoint Extraction for x,y,z,inten,r,g,b
vector<GrayValues*> keypoint_extract_xyzinten(vector<cv::KeyPoint> points,
                                              vector<GrayValues*> point_key,
                                              cv::Mat src) {
...
void print_newpoints(vector<Values*> newpoints) {
	...
	for (int i = 0; i < newpoints.size(); i++) {
		opfile << newpoints[i]->x << " " << newpoints[i]->y << " " <<
    newpoints[i]->z << " " << newpoints[i]->inten << " " << newpoints[i]->r <<
    " " << newpoints[i]->g << " " << newpoints[i]->b << endl;
	}
	opfile.close();
}
...
void print_newpoints_xyzinten(vector<GrayValues*> newpoints) {
	...
	for (int i = 0; i < newpoints.size(); i++) {
		opfile << newpoints[i]->x << " " << newpoints[i]->y << " " <<
    newpoints[i]->z << " " <<	newpoints[i]->inten << " " << endl;
	}
	opfile.close();
...
cv::Mat draw_points(cv::Mat src, vector<cv::KeyPoint> point) {
	...
	drawKeypoints(src, point, img, cv::Scalar(0, 0, 255),
...
cv::Mat forstner_xyzinten(cv::Mat src,
                          cv::Mat op,
                          vector<GrayValues*> points_of_interest) {
	float weight_mean;
	cv::Mat xder_kernel, yder_kernel;
	cv::Mat deriX, deriY; //Derivative of X & Y
	cv::Mat fxfx, fyfy, fxfy;//Elements of Structure Tensor
